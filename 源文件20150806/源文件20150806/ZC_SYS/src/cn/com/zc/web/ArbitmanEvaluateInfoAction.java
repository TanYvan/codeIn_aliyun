...
public class ArbitmanEvaluateInfoAction  extends BaseAction{
	...
	private HashMap<String,List<Evaluate>> assistantEvaluateHash;
	...
	public void setAssistantEvaluateHash(HashMap<String,List<Evaluate>> assistantEvaluateHash) {
		this.assistantEvaluateHash = assistantEvaluateHash;
	}
	
	public HashMap<String,List<Evaluate>> getAssistantEvaluateHash() {
		return assistantEvaluateHash;
	}
		...
		if (session.get("userType").toString().equals("arbitman") && session.get("receviceCode") != null){
			...
			if (rp.getStatus().equals("1")){
				...
				for(int i=0; i< caseArbitmanInList.size(); i++){
					...
					List<Evaluate> evaluateList = evaluateService.findEvaluateList(" and u=? and user_code=? and recevice_code=? and category='arbitman_to_arbitman' and used='Y' order by id asc", value, 0, 0);
			...
			if (rp.getStatus().equals("1")){
				...
				for(int i=0; i< caseWUserList.size(); i++){
					...
					partyEvaluateHash.put(caseWUserList.get(i).getUser_code(), evaluateService.findEvaluateList(" and u=? and user_code=? and recevice_code=? and category like 'party_to_arbitman%' and used='Y' order by id asc", value3, 0, 0));
			...
			assistantEvaluateHash = new HashMap<String,List<Evaluate>>();
			Object[] value4 = { session.get("userCode").toString(),session.get("receviceCode").toString() };
			assistantEvaluateHash.put("assistant" , evaluateService.findEvaluateList(" and user_code=? and recevice_code=? and category='assistant_to_arbitman' and used='Y' order by id asc", value4, 0, 0));
