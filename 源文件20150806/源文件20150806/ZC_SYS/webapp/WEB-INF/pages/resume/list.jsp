	...
	<body>
		<div align="center">
			...
			<table class="list_toolbar" cellspacing="0" cellpadding="0">
				<tr>
					<td align="left">
						<form action="${ctx}/resume/toInsertResume.do" method="post">
			...
			<table class="list_data_table" cellpadding="0" cellspacing="1">
				...
				<tbody>
					<%
						int i_tr = 0;
					%>
					<s:iterator value="resumeList" status="user">
						<%
							i_tr++;
						%>
						<tr class='<%=(i_tr % 2 == 0) ? "tr_even" : "tr_odd"%>'>
							...
							<td>
								<form
									action="${ctx}/resume/toUpdateResume.do?id=<s:property value='id'/>" method="post">
									<input type="submit" name="SubmitEdit" value="<s:text name="common.button_update"></s:text>" />
								</form>
							</td>
							<td>
								<form
									action="${ctx}/resume/deleteResume.do?id=<s:property value='id'/>" method="post" onsubmit="return confirm('<s:text name="common.button_delete_confirm"></s:text>');">
