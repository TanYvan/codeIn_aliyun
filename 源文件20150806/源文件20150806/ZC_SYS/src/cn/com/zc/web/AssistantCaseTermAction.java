...
public class AssistantCaseTermAction extends BaseAction {
	...
	public String list() throws Exception {
		if (session.get("userType").toString().equals("assistant")){
			...
			if (term_num != null && !"".equals(term_num)) {
				if (term_num.equals("0001")){
					sql = sql + " and TIMESTAMPDIFF(DAY, now(), tb_cases.finally_limit_dat)<=15";
				}else if (term_num.equals("0002")){
					sql = sql + " and TIMESTAMPDIFF(DAY, now(), tb_cases.finally_limit_dat)<=30 and TIMESTAMPDIFF(DAY, now(), tb_cases.finally_limit_dat)>15";
				}else if (term_num.equals("0003")){
					sql = sql + " and TIMESTAMPDIFF(DAY, now(), tb_cases.finally_limit_dat)<=60 and TIMESTAMPDIFF(DAY, now(), tb_cases.finally_limit_dat)>30";
				}else if (term_num.equals("0004")){
					sql = sql + " and TIMESTAMPDIFF(DAY, now(), tb_cases.finally_limit_dat)>60";
				}
				params.add(new Param("term_num",term_num));
			}
			...
			if (rp.getStatus().equals("1")){
				...
				page_bar = page_bar(rp,request.getContextPath() + "/assistantCaseTerm/list.do","list_page_bar");
				return SUCCESS;
			}
			else{
				return "fail";
