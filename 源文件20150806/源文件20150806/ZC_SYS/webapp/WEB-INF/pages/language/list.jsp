	...
	<body>
		<div align="center">
			...
			<table class="list_toolbar" cellspacing="0" cellpadding="0">
				<tr>
					<td align="left">
						<form action="${ctx}/language/toInsertLanguage.do" method="post">
			...
			<table class="list_data_table" cellpadding="0" cellspacing="1">
				...
				<tbody>
					<%
						int i_tr = 0;
					%>
					<s:iterator value="languageList" status="user">
						<%
							i_tr++;
						%>
						<tr class='<%=(i_tr % 2 == 0) ? "tr_even" : "tr_odd"%>'>
							...
							<td>
								<form
									action="${ctx}/language/toUpdateLanguage.do?id=<s:property value='id'/>" method="post">
									<input type="submit" name="SubmitEdit" value="<s:text name="common.button_update"></s:text>" />
								</form>
							</td>
							<td>
								<form
									action="${ctx}/language/deleteLanguage.do?id=<s:property value='id'/>" method="post" onsubmit="return confirm('<s:text name="common.button_delete_confirm"></s:text>');">
