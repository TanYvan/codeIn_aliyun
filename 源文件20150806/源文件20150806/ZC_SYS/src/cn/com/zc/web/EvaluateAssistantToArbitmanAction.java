...
public class EvaluateAssistantToArbitmanAction  extends BaseAction{
	...
	public String findEvaluateAssistantToArbitmanList() throws Exception{
		...
		if (session.get("userType").toString().equals("assistant") && session.get("receviceCode") != null){
			...
			if (rp.getStatus().equals("1")){
				...
				for(int i=0; i< caseArbitmanInList.size(); i++){
					...
					evaluateHash.put(caseArbitmanInList.get(i).getArbitman(), evaluateService.findEvaluateList(" and u=? and user_code=? and recevice_code=? and category='assistant_to_arbitman' and used='Y' order by id asc", value, 0, 0));
	...
	public String toInsertEvaluateAssistantToArbitman() throws Exception {
		if (recevice_code == null && session.get("receviceCode") == null){
			return "select_case";
		}
	...
	public String insertEvaluateAssistantToArbitman() throws Exception {
		if (recevice_code == null && session.get("receviceCode") == null){
			return "select_case";
		}
	...
	public String toUpdateEvaluateAssistantToArbitman() throws Exception {
		if (recevice_code == null && session.get("receviceCode") == null){
			return "select_case";
		}
	...
	public String updateEvaluateAssistantToArbitman() throws Exception {
		if (recevice_code == null && session.get("receviceCode") == null){
			return "select_case";
		}
		...
		if (session.get("userType").toString().equals("assistant") && session.get("receviceCode") != null){
			...
			List<Evaluate> evaList = evaluateService.findEvaluateList(" and u=? and recevice_code=? and category='assistant_to_arbitman' and used='Y' order by id asc", value, 0, 0);
