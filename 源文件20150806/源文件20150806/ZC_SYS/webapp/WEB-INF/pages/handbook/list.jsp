	...
	<body>
		<div align="center">
			...
			<table class="list_title" cellspacing="0" cellpadding="0">
				<tr>
					<td>
						<span class="list_title_span">办案指南信息管理</span>
					</td>
				</tr>
				<tr>
			...
			<form action="${ctx}/handbook/list.do" method="post">
				<table class="list_search_table" cellspacing="0" cellpadding="0">
					<tr>
						<td>
							类型:
							<s:select id="category" name="category"
								list="baseDicAllList['9035']" listKey="dataVal" listValue="dataText"  ></s:select>
							发布范围:
			...
			<table class="list_toolbar" cellspacing="0" cellpadding="0">
				<tr>
					<td align="left">
						<form action="${ctx}/handbook/toInsert.do" method="post">
				...
				<tbody>
					...
					<s:iterator value="handbookList" status="role">
						...
						<tr class='<%=(i_tr % 2 == 0) ? "tr_even" : "tr_odd"%>'>
							...
							<td>
								<form
									action="${ctx}/handbook/toUpdate.do?id=<s:property value='id'/>"
									method="post">
					...
					<td>
						...
						<s:if test="pageNo != 1">
							<a class="list_page_bar_link"
								href='${ctx}/handbook/list.do?pageNo=1${page_str}${page_str}'>首页</a>
						</s:if>
						<s:if test="pageNo > 1">
							<a class="list_page_bar_link"
								href='${ctx}/handbook/list.do?pageNo=${pageNo-1}${page_str}'>上一页</a>
						</s:if>
						<s:if test="pageNo < pageCount">
							<a class="list_page_bar_link"
								href='${ctx}/handbook/list.do?pageNo=${pageNo+1}${page_str}'>下一页</a>
						</s:if>
						<s:if test="pageNo!=pageCount">
							<a class="list_page_bar_link"
								href='${ctx}/handbook/list.do?pageNo=${pageCount}${page_str}'>末页</a>
						</s:if>
					</td>
				</tr>
			</table>
