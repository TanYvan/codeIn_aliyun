...
public class PartyAction extends BaseAction {
	...
	public String insertParty() {
		...
		if (session.get("applyCode").toString() != null){
			...
			error = "申请人信息新建成功";
			error = this.getText("party.new.mes_1");
			return SUCCESS;
		}else{
			return "fail";
		}
	...
	public String toUpdateParty() {
		Object[] value = {id};
		partyList = partyService.findPartyList(" and id=? and partytype=1 and used='Y'", value, 0, 0);
		party = partyList.get(0);
		if (party.getApplyCode().equals(session.get("applyCode").toString()) && party.getPartyType()==1){
			return SUCCESS;
		}else{
			return "fail";
		}
	...
	public String deleteParty() {
		...
		if (party_up.getApplyCode().equals(session.get("applyCode").toString()) && party_up.getPartyType()==1){
			...
			partyService.updateParty(party_up);
			error = "申请人信息删除成功";
			error = this.getText("party.new.mes_3");
			return SUCCESS;
		}
		else{
			return "fail";
