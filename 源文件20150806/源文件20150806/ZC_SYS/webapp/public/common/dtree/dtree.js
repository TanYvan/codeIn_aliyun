...
dTree.prototype.node = function(node, nodeId) {
  ...
  if (node.hasCheckBox){
    str += '<input type=checkbox name=\"treeCheckBox\" id=\"' + node.id + '\" onclick="changeSelect(\'' +node.id + '\')" value=\"' + node.id + '\"';
...
dTree.prototype.getSubNodesById = function(id){
	var array = [];
	var i = 0;
	for(var n=0;n<this.aNodes.length;n++){
		if(this.aNodes[n].pid == id ){
			array[i] = this.aNodes[n];
			i++;
		}
	}
	return array;
