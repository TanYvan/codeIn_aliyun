...
public class SearchArbitmanInForUserAction extends BaseAction {
	...
	public String findArbitmanInForUserList() throws Exception {
		...
		if (rp.getStatus().equals("1")){
			...
			page_bar = page_bar(rp,request.getContextPath() + "/searchArbitmanInForUser/findArbitmanInForUserList.do","list_page_bar");
			return SUCCESS;
		}
		else{
			return "fail";
	...
	public String showArbitman() throws Exception {
	...
	public String select() throws Exception {
		...
		req.setConditions(conditions);
		req.setParams(params);
		req.setOrder("name_idcard asc,name_idcard_en asc");
		if (page!=null){
			req.setPage(page);
		}
		if (perpage!=null){
		...
		if (rp.getStatus().equals("1")){
			arbitmanInList = new ArbitmanIn().getObjects(rp);
			page_bar = page_bar(rp,request.getContextPath() + "/searchArbitmanInForUser/select.do","list_page_bar");
			return SUCCESS;
		}
		else{
			return "fail";
