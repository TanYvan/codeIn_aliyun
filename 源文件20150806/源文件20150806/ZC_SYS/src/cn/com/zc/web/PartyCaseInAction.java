...
public class PartyCaseInAction extends BaseAction {
	...
	public String getReturnUrl() {
		if (returnUrl==null || returnUrl.equals("")){
			this.returnUrl="/partyCaseIn/showPartyCaseIn.do";
		}
		return returnUrl;
	}
	...
	public String findPartyCaseInList() throws Exception {
		if (session.get("userType").toString().equals("party")){
			ReqPacket req= new ReqPacket();
			req.setFrom("tb_cases,case_w_user,users");
			req.setSelect("tb_cases.recevice_code as recevice_code,tb_cases.case_code as case_code,tb_cases.accepttime as accepttime,tb_cases.case_code as case_code,tb_cases.aribitprog_num as aribitprog_num,tb_cases.clerk as clerk,users.name as clerk_name,tb_cases.state as state,tb_cases.finally_limit_dat as finally_limit_dat,fun_partyinfo_1(tb_cases.recevice_code) as partyinfo_1,fun_partyinfo_2(tb_cases.recevice_code) as partyinfo_2,tb_cases.caseendbook_id_first as caseendbook_id_first");
			ArrayList<Param> params = new ArrayList<Param>();
			params.add(new Param("returnUrl",returnUrl));
			req.setParams(params);
			List<String> conditions =  new ArrayList<String>();
			conditions.add("case_w_user.user_code=? and case_w_user.used='Y' and tb_cases.recevice_code=case_w_user.recevice_code and tb_cases.used='Y' and tb_cases.state>=4 and tb_cases.clerk=users.code ");
			conditions.add(session.get("userCode").toString());
			req.setConditions(conditions);
			...
			if (rp.getStatus().equals("1")){
				...
				page_bar = page_bar(rp,request.getContextPath() + "/partyCaseIn/findPartyCaseInList.do","list_page_bar");
				return SUCCESS;
			}
			else{
				return "fail";
	...
	public String showPartyCaseIn() throws Exception {
		...
		if (session.get("userType").toString().equals("party") && session.get("receviceCode") != null ){
			...
			req.setSelect("recevice_code, case_code, clerk, fun_case_clerk_name(recevice_code) as clerk_name, fun_case_clerk_2_name(recevice_code) as clerk_2_name, t_casetype_num, dispute_type, aribitprog_num, acceptt, aribittype, case_type1, trial_typ, casetype_num, casetype_num2, accepttime, agent, arbitprot, special, casereason, clerk_2, casereason, limit_dat, finally_limit_dat, runstyle, runremark,caseendbook_id_first");
			List<String> conditions =  new ArrayList<String>();
